...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  return {
    title: `${name} Mujer — SAVAYA`,
    description: `Colección femenina ${name.toLowerCase()} — SAVAYA Calzado venezolano`,
    alternates: {
      canonical: `${BASE_URL}/mujer/categoria/${slug}`,
    },
    ...(noindex && { robots: { index: false, follow: true } }),
  }
}

export default async function MujerCategoryPage({ params, searchParams }: Props) {
  const { slug } = await params
  const rawParams = await searchParams
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Mujer', item: `${BASE_URL}/mujer` },
