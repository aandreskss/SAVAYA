...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  return {
    title: `${name} Hombre — SVY FOR MEN | SAVAYA`,
    description: `Colección masculina ${name.toLowerCase()} — SVY FOR MEN by SAVAYA`,
    alternates: {
      canonical: `${BASE_URL}/hombre/categoria/${slug}`,
    },
    ...(noindex && { robots: { index: false, follow: true } }),
  }
}

export default async function HombreCategoryPage({ params, searchParams }: Props) {
  const { slug } = await params
  const rawParams = await searchParams
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Hombre', item: `${BASE_URL}/hombre` },
    ...
    <>
      ...
      <div className="max-w-screen-xl mx-auto px-4 md:px-10 py-8">
        ...
        <div className="mt-6 mb-8">
          {/* SVY FOR MEN wordmark */}
          <div className="mb-3">
            <span className="relative block w-[140px] h-[40px]">
              <Image
          ...
          <div className="flex-1 min-w-0">
            ...
            {items.length === 0 ? (
              <EmptyState
                ...
                description="Prueba con otros filtros o explora toda nuestra colección masculina"
              />
            ) : (
              <div
                className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6"
                aria-label={`Productos masculinos de ${category.name}`}
              >
