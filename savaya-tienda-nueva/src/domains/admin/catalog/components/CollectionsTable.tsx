...
export function CollectionsTable({ rows }: Props) {
  ...
  function confirmDelete() {
    ...
    startTransition(async () => {
      ...
      else toast.success('Colección eliminada')
    })
  }
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        <Link href="/admin/productos/colecciones/nuevo">
          <Button size="sm" leftIcon={<PlusIcon />}>Nueva colección</Button>
        </Link>
      </div>

      {rows.length === 0 ? (
        <div className="border border-border rounded-xl p-12 text-center bg-surface">
          <p className="font-sans text-sm text-text-secondary">Sin colecciones todavía.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm" role="grid">
            <thead className="bg-surface-2 border-b border-border">
              <tr>
                ...
                <th scope="col" className="px-4 py-3 text-left font-sans text-xs font-medium text-text-secondary uppercase tracking-wider">Destacada</th>
      ...
      <Modal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Eliminar colección" size="sm">
        <div className="space-y-4">
          <p className="font-sans text-sm text-text-primary">
            ¿Eliminar <strong>{deleteTarget?.name}</strong>? Los productos no se eliminarán, solo se desasociarán de esta colección.
          </p>
